...
static void drawWeatherSymbol(u8g2_uint_t x, u8g2_uint_t y, uint8_t symbol)
{
  ...
  {
    case SUN:
      u8g2.setFont(u8g2_font_open_iconic_weather_6x_t);         //u8g2.setFont用于设置字体
      u8g2.drawGlyph(x, y, 69);                                 //u8g2.drawGlyph用于绘制特殊图标
      break;
    case SUN_CLOUD:
...
{
  drawWeatherSymbol(0, 63, symbol);                     //绘制图标
  u8g2.setFont(u8g2_font_logisoso32_tf);                //设置字体
...
void oled_display()
{
    ...
    u8g2.drawStr(30, 60, "By Charlie");   // write something to the internal memory
    u8g2.sendBuffer();
